...
7. the current protected `runethread/core/RUNETHREAD_INVARIANTS.json`, relevant accepted Core ADRs, and `docs/runethread/VERSIONING.md` when the change affects version/release identity, all from freshly verified live state;
...
Record the exact Core invariant-authority commit used for a substantive change. Scope means applicability, not proof: Hosted must evaluate its own source against applicable invariant IDs and must not copy a competing project registry. When release/version identity is in scope, also record the exact Core versioning-authority commit consumed by Hosted.

Conversation history and handoffs are orientation only. If live GitHub state, current Core authority, release identity policy, versioning authority, or provider documentation contradicts the plan, stop writes and resolve the discrepancy first.
...
- ADR-028 and Core `docs/runethread/VERSIONING.md` are the project-wide component-versioning authority. Hosted has its own independent SemVer release line; matching Core/Hosted/adapter numbers never imply compatibility.
- Treat the raw Semantic Versioning 2.0.0 value and Runethread's public release identifier as distinct representations: for example raw `0.3.0` and public identifier/tag `v0.3.0`. Do not treat the project `v` prefix as part of the SemVer value itself.
- `release/identity-policy.json` is the admitted Hosted release-compatibility baseline. It pins the exact Core versioning-authority commit/blob plus every correctness-relevant Core/runtime/toolchain/provider identity; never substitute floating development refs.
...
- CI uses reserved release identifier `v0.0.0-ci` / SemVer value `0.0.0-ci` only to prove release-identity machinery. It is not a Hosted product release.
- A generated release-instance manifest binds release identifier plus raw SemVer value, source commit/tree, versioning-policy identity, Core/runtime/contract identity, provider/build inputs, executable Worker digest, protocol/capability state, and required notice-file digests. It does not by itself prove protected-branch/ruleset/status-check state.
